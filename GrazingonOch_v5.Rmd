...
Figure 1: Ingestion rates

```{r Figure 1: Ingestion Rates,fig.width=6,fig.height=5}
...
Revision of Figure 1 -- making points more transparent
```{r Figure 1R: Ingestion Rates,fig.width=6,fig.height=5}
...
xlims <- c(0,165000)
ylims <- matrix(c(19,18,73,45),nrow=2,ncol=2,byrow=T)
#ylims <- matrix(c(19,19,73,73),nrow=2,ncol=2,byrow=T)
...
subdat <- subdat[subdat$prey.i.24>=0,]
  ...
  # RE PLOT POINTS SO THEY ARE ON TOP
  points((subdat$prey.popn.24),subdat$prey.i.24,pch=pchs[as.factor(subdat$Rep)],lwd=1,col=bg2use,bg=bg2use)
...
Also making the y-axes the same
```{r Figure 1R2: Ingestion Rates,fig.width=6,fig.height=5}
...
signif.mat <- matrix(c('a','a,b','b','a','a','a','a','a','b','a','a','a'),nrow=4,ncol=3,byrow=T)
#signif.y <- matrix(c(1.95,1.2,1.1,1.5,2.95,1.08,.55,1.03,.4,.52,.4,.32),nrow=4,ncol=3,byrow=T)
signif.y <- matrix(c(2,1.25,1.17,1.5,2.95,1.1,.57,1.03,.4,.56,.55,.43),nrow=4,ncol=3,byrow=T)

#y.lim.min <- matrix(c(0.38,0.4,0.18,0.18),nrow=2,ncol=2,byrow=T)
#y.lim.max <- matrix(c(2,3,1.05,.55),nrow=2,ncol=2,byrow=T)
y.lim.min <- matrix(c(0.38,0.38,0.18,0.18),nrow=2,ncol=2,byrow=T)
y.lim.max <- matrix(c(3,3,1.05,1.05),nrow=2,ncol=2,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  if(strainctr==1){
    if(lightctr==2){
      mtext(expression(paste('Clearance rate (μL · ',predator^-1,' · ',day^-1,')')),side=2,line=2.5,cex=.8,at=-0.2)
    }
  }
...
#y.lim.min <- matrix(c(.8,.5,0,0),nrow=2,ncol=2,byrow=T)
#y.lim.max <- matrix(c(2.9,3.3,.36,.9),nrow=2,ncol=2,byrow=T)
y.lim.min <- matrix(c(.5,.5,0,0),nrow=2,ncol=2,byrow=T)
y.lim.max <- matrix(c(3.3,3.3,.85,.85),nrow=2,ncol=2,byrow=T)
signif.y <- matrix(c(2.25,2.35,2.85,2.6,3.25,2.12,.1,.32,.4,.82,.322,.292),nrow=4,ncol=3,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  if(rowcount == 3){
    boxplot(Best.h.2*24~EvoTemp,data=fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,],las=1,xaxt='n',ylab='',xlab='',col=EvoBgCols,border=EvoCols,ylim=c(y.lim.min[strainctr,lightctr],y.lim.max[strainctr,lightctr])) #,yaxt='n'
    #axis(side = 2, at = c(0,0.1,0.2,0.3),las=1)
  } else{
  boxplot(Best.h.2*24~EvoTemp,data=fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,],las=1,xaxt='n',ylab='',xlab='',col=EvoBgCols,border=EvoCols,ylim=c(y.lim.min[strainctr,lightctr],y.lim.max[strainctr,lightctr]))}
  axis(side=1,at=c(1:3),labels=F)
...
Supplemental figure comparing across temperatures

```{r SFig: Temp Comparison,fig.width=6,fig.height=5}
...
```{r Supp Fig: Temp comparison revised,fig.width=6,fig.height=5}
...
# Axis limits
xlims <- c(0,148000)
ylims <- matrix(c(42,42,75,75),nrow=2,ncol=2,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  if(strainctr==1){
    if(lightctr==2){
      mtext(expression(paste('Clearance rate (μL · ',predator^-1,' · ',d^-1,')')),side=2,line=2.4,cex=.8,at=0)
    }
  }
...
Overlay predator normalized growth responses (similar to #1 above; 2x2 panel)
...
Revised plot
```{r Figure2: Predator growth R1,fig.width=5.5,fig.height=5.4}
...
# Axis limits
xlims <- c(0,165000)
ylims <- matrix(c(.3,.3,.3,.3),nrow=2,ncol=2,byrow=T)
ylims.low <- matrix(c(-.45,-.45,-.18,-.18),nrow=2,ncol=2,byrow=T)
...
if(lightctr==2){
  axis(side=2,las=1)
  if(strainctr==2){
    legend(129000,0.33,c('18°C','24°C','30°C'),pch=21,col=EvoCols,pt.bg=EvoBgCols,lwd=1,bty='n',bg='white')  }
}
if(strainctr==2){
  if(lightctr==2){
    axis(side=1,las=1)
...
subdat <- subdat[subdat$prey.i.24>=0,]
  ...
  # RE PLOT POINTS SO THEY ARE ON TOP
  points((subdat$prey.popn.24),subdat$pred.sens.72,pch=pchs[as.factor(subdat$Rep)],lwd=1,col=bg2use,bg=bg2use)
...
#box.ylims.min <- matrix(c(-3.4,-3.1,0,-.45),nrow=2,ncol=2,byrow=T)
#box.ylims.max <- matrix(c(3.25,.7,2.8,2.5),nrow=2,ncol=2,byrow=T)
box.ylims.min <- matrix(c(-3.4,-3.4,-.45,-.45),nrow=2,ncol=2,byrow=T)
box.ylims.max <- matrix(c(3.25,3.25,2.8,2.8),nrow=2,ncol=2,byrow=T)

# Significance letters
signif.mat <- matrix(c('a','a','a','a','a','b','a','a','a','a','a','a'),nrow=4,ncol=3,byrow=T)
#signif.y <- matrix(c(2.3,3.2,.6,.465,.3,-.6,2.7,2.6,1.85,2,2.5,1.3),nrow=4,ncol=3,byrow=T)
signif.y <- matrix(c(2.3,3.2,.6,.7,.6,1.25,2.75,2.65,1.91,2,2.54,1.33),nrow=4,ncol=3,byrow=T)
signif.t <- matrix(c('','','','*','*','','**','***','***','','**',''),nrow=4,ncol=3,byrow=T)
#signif.t.y <- matrix(c(0,0,0,-3.1,-2.9,0,.46,.46,.46,0,-.35,0),nrow=4,ncol=3,byrow=T)
signif.t.y <- matrix(c(0,0,0,-3.2,-3,0,.46,.46,.46,0,-.33,0),nrow=4,ncol=3,byrow=T)
...
Tukey tests
```{r}
TukeyHSD(aov(pgC.cell~as.factor(Ev)*as.factor(Strain),data=dat50.CN))
TukeyHSD(aov(pgN.cell~as.factor(Ev)*as.factor(Strain),data=dat50.CN))
TukeyHSD(aov(pgC.cell/pgN.cell~as.factor(Ev)*as.factor(Strain),data=dat50.CN)) # no significant differences

TukeyHSD(aov(pgC.cell~as.factor(Ev)*as.factor(Strain),data=dat100.CN))
TukeyHSD(aov(pgN.cell~as.factor(Ev)*as.factor(Strain),data=dat100.CN))
TukeyHSD(aov(pgC.cell/pgN.cell~as.factor(Ev)*as.factor(Strain),data=dat100.CN)) # no significant differences
```

Revised for main text
```{r,fig.height=6,fig.width=5.5}
par(mfcol=c(3,2))
xcoords <- c(1,2.05,3.1,4.6,5.65,6.7)

par(mar=c(0,4,4,1))
bargraph.CI(Strain,pgC.cell,group=Ev,data=dat50.CN,las=1,legend=F,col=EvoBgCols,border=EvoCols,space=c(0.05,.5),ylab='',err.width=0.05,names=c('',''),main=expression(paste('Low light (50 μmol quanta · ',m^-2,' ',s^-1,')' )),ylim=c(0,23.5)); mtext(expression(paste('Cellular carbon (pg C · ',cell^-1,')')),side=2,line=2.5,cex=.7)
signifs <- c('a','b,c','b,c','c','c','a,b')
text(xcoords,c(23,16.5,17.3,14.2,15,21.3),signifs)

par(mar=c(2,4,2,1))
bargraph.CI(Strain,pgN.cell,group=Ev,data=dat50.CN,las=1,col=EvoBgCols,border=EvoCols,space=c(0.05,.5),ylab='',err.width=0.05,names=c('',''),ylim=c(0,3.05)); mtext(expression(paste('Cellular nitrogen (pg N · ',cell^-1,')')),side=2,line=2.5,cex=.7)
signifs <- c('a','a,b','a,b','b','a,b','a')
text(xcoords,c(3,2.45,2.57,2.25,2.45,3),signifs)

par(mar=c(4,4,0,1))
bargraph.CI(Strain,pgC.cell/pgN.cell,group=Ev,data=dat50.CN,las=1,col=EvoBgCols,border=EvoCols,space=c(0.05,.5),ylab='',err.width=0.05,names=c('Obligate (1391)','Facult. (2951)'),xlab=''); mtext(expression(paste('C:N ratio (pg C · pg ',N^-1,')')),side=2,line=2.5,cex=.7); mtext(expression(paste(italic('Ochromonas'),' strain')),side=1,line=2.5,cex=.7)

par(mar=c(0,2,4,3))
bargraph.CI(Strain,pgC.cell,group=Ev,data=dat100.CN,las=1,legend=F,col=EvoBgCols,border=EvoCols,space=c(0.05,.5),ylab='',err.width=0.05,names=c('',''),main=expression(paste('High light (100 μmol quanta · ',m^-2,' ',s^-1,')' )),ylim=c(0,21)); legend(x='topright',inset=c(0.0,-.06),legend=c('18°C','24°C','30°C'),col=EvoCols,pt.bg=EvoBgCols,pch=22,pt.cex=2,bty='n',cex=1.1)
signifs <- c('a','b','b','b','c','c')
text(xcoords,c(20.6,15.6,14.4,14.9,12.6,12.1),signifs)

par(mar=c(2,2,2,3))
bargraph.CI(Strain,pgN.cell,group=Ev,data=dat100.CN,las=1,col=EvoBgCols,border=EvoCols,space=c(0.05,.5),ylab='',err.width=0.05,names=c('',''),ylim=c(0,3.87))
signifs <- c('a','b','b,c','b,c','c','b,c')
text(xcoords,c(3.8,2.65,2.3,2.26,2.05,2.15),signifs)
...
```{r Fig: Carbon Ingestion vs Growth Revision, fig.height=7,fig.width=7}

# Labels
labels <- matrix(c('(a) Obligate, low light','(b) Obligate, high light','(c) Facultative, low light','(d) Facultative, high light'),nrow=2,ncol=2,byrow=T)

col18bg2 <- rgb(34/255,91/255,178/255,.7) # OKeeffe1
col24bg2 <- rgb(0,0,0,.3)#'gray80'
col30bg2 <- rgb(150/255,61/255,32/255,.7) # OKeeffe1


EvoCols <- c(col18,col24,col30)
EvoBgCols2 <- c(col18bg2,col24bg2,col30bg2)
...
# Add points first so they're on the bottom
points(subdat$prey.i.24.C,subdat$pred.mu.72 ,col=EvoBgCols2[as.factor(subdat$EvoTemp)],pch=pchs[as.factor(subdat$Rep)],bg=EvoBgCols2[as.factor(subdat$EvoTemp)],cex=1.5,lwd=1)
...
## Ingestion as a function of carbon

Calculating ingestion and prey density in terms of pg C
```{r}
# Convert ingestion to carbon
dat$prey.i.24.C <- dat$prey.i.24*dat$pgC.cell

# Convert prey abundance to carbon
dat$prey.popn.24.C <- dat$prey.popn.24*dat$pgC.cell/1000/1000
dat$prey.popn.24.pgC <- dat$prey.popn.24*dat$pgC.cell
dat$D1OchPop.C <- dat$D1OchPop*dat$pgC.cell

```

Populate data table to hold results
```{r}
# Create columns to hold Type I results
fits$TypeI.a.C <- NaN
fits$TypeI.a.p.C <- NaN
fits$TypeI.a.err.C <- NaN
fits$TypeI.AIC.C <- NaN

# Create columns to hold Type II results
fits$TypeII.a.C <- NaN
fits$TypeII.a.p.C <- NaN
fits$TypeII.a.err.C <- NaN
fits$TypeII.h.C <- NaN
fits$TypeII.h.p.C <- NaN
fits$TypeII.h.err.C <- NaN
fits$TypeII.AIC.C <- NaN

# Create columns to hold the 'best choice' results
fits$Best.a.C <- NaN
fits$Best.h.C <- NaN
fits$BestModel.C <- NaN
...
for(i in 1:length(expts)){
  ...
  if(fits[fits$Expt==expts[i],]$AssayTemp[1] %in% c(18,24)){
    ...
    # Data cleaning: Remove negative values and outliers
    subdat <- subdat[subdat$prey.i.24.C>=0,] # (removes negative values)
    OutVals = boxplot(subdat$prey.i.24.C~subdat$TargetOchConc,plot=F)$out # (identifies outliers)
    if(length(OutVals > 0)){ subdat <- subdat[which(subdat$prey.i.24.C != OutVals),] } # (removes outliers; note use of "if" statement so you don't crash if there are no outliers)

    # Rep-by-Rep fitting
    for(j in 1:length(reps)){
      subdat2 <- subdat[subdat$Rep==reps[j],]
      # TYPE I
      lm1 <- nls(prey.i.24.C~a*prey.popn.24.pgC,data=subdat2,start=list(a=a.est))
      fits[fits$ID == subdat2$ID[1],]$TypeI.a.C <- summary(lm1)$coefficients[1,1]
      fits[fits$ID == subdat2$ID[1],]$TypeI.a.err.C <- summary(lm1)$coefficients[1,2]
      fits[fits$ID == subdat2$ID[1],]$TypeI.a.p.C <- summary(lm1)$coefficients[1,4]
      fits[fits$ID == subdat2$ID[1],]$TypeI.AIC.C <- AIC(lm1) + 1 # Penalize the AIC for the linear model by a point because I forced it through zero, so it's fitting one less parameter. (This doesn't seem to make much of a difference.)
  
      # TYPE II
      a.est <- summary(lm(subdat2[1:3,]$prey.i.24.C~subdat2[1:3,]$prey.popn.24.pgC))$coefficients[2,1] # estimate initial slope
      h.est <- 1/max(subdat2$prey.i.24.C) # estimate handling time
      if( subdat2$ID[1]=="33.F"){
    fit1 <- nls(prey.i.24.C ~ a*D1OchPop.C/(1+a*h*D1OchPop.C),data=subdat2,start = list(a = a.est, h = h.est))
    } else{
      fit1 <- nls(prey.i.24.C ~ a*prey.popn.24.pgC/(1+a*h*prey.popn.24.pgC),data=subdat2,start = list(a = a.est, h = h.est))
    }
      fits[fits$ID == subdat2$ID[1],]$TypeII.a.C <- summary(fit1)$parameters[1,1]
      fits[fits$ID == subdat2$ID[1],]$TypeII.a.err.C <- summary(fit1)$parameters[1,2]
      fits[fits$ID == subdat2$ID[1],]$TypeII.a.p.C <- summary(fit1)$parameters[1,4]
      fits[fits$ID == subdat2$ID[1],]$TypeII.h.C <- summary(fit1)$parameters[2,1]
      fits[fits$ID == subdat2$ID[1],]$TypeII.h.err.C <- summary(fit1)$parameters[2,2]
      fits[fits$ID == subdat2$ID[1],]$TypeII.h.p.C <- summary(fit1)$parameters[2,4]
      fits[fits$ID == subdat2$ID[1],]$TypeII.AIC.C <- AIC(fit1)
      
      
      # Choose the best model
      if( fits[fits$ID == subdat2$ID[1],]$TypeI.AIC.C < fits[fits$ID == subdat2$ID[1],]$TypeII.AIC.C ){ # if the Type I model is better
        fits[fits$ID == subdat2$ID[1],]$Best.a.C <- fits[fits$ID == subdat2$ID[1],]$TypeI.a.C
        fits[fits$ID == subdat2$ID[1],]$Best.h <- 0
        fits[fits$ID == subdat2$ID[1],]$BestModel.C <- 'Type I'
      } else{
        if( fits[fits$ID == subdat2$ID[1],]$TypeII.h.C < 0 ){ # if handling time is negative
          fits[fits$ID == subdat2$ID[1],]$Best.a.C <- fits[fits$ID == subdat2$ID[1],]$TypeI.a.C
          fits[fits$ID == subdat2$ID[1],]$Best.h.C <- 0
          fits[fits$ID == subdat2$ID[1],]$BestModel.C <- 'Type I'
        } else {
          fits[fits$ID == subdat2$ID[1],]$Best.a.C <- fits[fits$ID == subdat2$ID[1],]$TypeII.a.C
          fits[fits$ID == subdat2$ID[1],]$Best.h.C <- fits[fits$ID == subdat2$ID[1],]$TypeII.h.C
          fits[fits$ID == subdat2$ID[1],]$BestModel.C <- 'Type II'
        }
      }
      } # End looping over reps
    
    # Global fit
    a.est <- summary(lm(subdat[1:7,]$prey.i.24.C~subdat[1:7,]$prey.popn.24.pgC))$coefficients[2,1] # estimate initial slope
    h.est <- 1/max(subdat$prey.i.24.C,na.rm=T) # estimate handling time
    fit2 <- nls(prey.i.24.C ~ a*prey.popn.24.pgC/(1+a*h*prey.popn.24.pgC),data=subdat,start = list(a = a.est, h = h.est))
    fits[fits$Expt == expts[i],]$Glob.a.C <- rep(summary(fit1)$parameters[1,1],length(reps))
    fits[fits$Expt == expts[i],]$Glob.h.C <- rep(summary(fit1)$parameters[2,1],length(reps))
...
# Force handling time to zero if using Type I fit
fits$Best.h.2.C <- fits$Best.h.C
fits[is.na(fits$Best.h.2.C),]$Best.h.2.C <- 0


```

And new figure, showing ingestion as a function of prey density, but all in units of carbon
...
xlims <- c(0,2.7)
#ylims <- matrix(c(19,18,73,45),nrow=2,ncol=2,byrow=T)
ylims <- matrix(c(400,400,900,900),nrow=2,ncol=2,byrow=T)
...
plot((subdat$prey.popn.24.C),subdat$prey.i.24.C,las=1,xlab='',ylab='',type='n',xlim=xlims,ylim=c(0,ylims[strainctr,lightctr]),xaxt='n')
axis(side=1,las=1,labels=F)
#axis(side=2,las=1,labels=F)
...
if(lightctr==2){
  axis(side=2,las=1)
  if(strainctr==2){
    legend(1.95,950,c('18°C','24°C','30°C'),pch=21,col=EvoCols,pt.bg=EvoBgCols,lwd=1,bty='n')  }
}
if(strainctr==2){
  if(lightctr==2){
    axis(side=1,las=1)
    mtext(expression(paste('Mean ', italic('Ochromonas'),' biomass (mg C · ',mL^-1,')')),side=1,line=2.2,cex=.8)
  }
}

  if(strainctr == 1){
    if(lightctr==2){
    mtext(expression(paste('Ingestion rate (pg ',italic('Ochromonas'),' C · ',italic('O. marina')^-1,' · ',d^-1,')')),side=2,line=2.3,at=-2,cex=.8)
  }}
...
for(tempchoice in 1:length(EvoTemps)){
  ...
  subdat <- dat[dat$Pred!='Control'& dat$Strain==StrainChoice & dat$EvoLight==EvoLightChoice & dat$EvoTemp==EvoTempChoice & dat$AssayTemp==AssayTempChoice,]
subdat <- subdat[subdat$prey.i.24.C>=0,]

  # Use a boxplot to identify outliers
  OutVals = boxplot(subdat$prey.i.24.C~subdat$TargetOchConc,plot=F)$out

  # Remove those outliers from the dataset
  if(length(OutVals > 0)){ subdat <- subdat[which(subdat$prey.i.24.C != OutVals),] }
  ...
  for(i in 1:length(reps)){
    ...
    # TYPE II
    if( subdat2$ID[1]=="33.F"){
    #fit1 <- nls(prey.i.24.C ~ a*D1OchPop/(1+a*h*D1OchPop),data=subdat2,start = list(a = a.est, h = h.est))
    AIC.II <- AIC(fit1)
    } else{
    a.est <- summary(lm(subdat2[1:3,]$prey.i.24.C~subdat2[1:3,]$prey.popn.24.C))$coefficients[2,1] # estimate initial slope
    h.est <- 1/max(subdat2$prey.i.24.C) # estimate handling time
    fit1 <- nls(prey.i.24.C ~ a*prey.popn.24.C/(1+a*h*prey.popn.24.C),data=subdat2,start = list(a = a.est, h = h.est))
    AIC.II <- AIC(fit1)
    }
  
    # TYPE I
    lm1 <- nls(prey.i.24.C~a*prey.popn.24.C,data=subdat2,start=list(a=a.est))
    AIC.I <- AIC(lm1) + 1 # Penalize the AIC for the linear model by a point because I forced it through zero, so it's fitting one less parameter. (This doesn't seem to make much of a difference.)
    ...
    if(h.fit < 0 ){ 
      s.fit <- summary(lm1)$parameters[1,1]
      xcoords <- seq(from = 0, to = max(subdat2$prey.popn.24.C,na.rm=T),length.out = 100)
      ycoords <- s.fit*xcoords
      lines(xcoords,ycoords,lwd=1,col=bg2use)
    #abline(a=0,b=s.fit,col=bg2use,lwd=2)
    }else{
  ...
  # RE PLOT POINTS SO THEY ARE ON TOP
  points((subdat$prey.popn.24.C),subdat$prey.i.24.C,pch=pchs[as.factor(subdat$Rep)],lwd=1,col=bg2use,bg=bg2use)

# OVERALL FIT
  a.est <- summary(lm(subdat[1:7,]$prey.i.24.C~subdat[1:7,]$prey.popn.24.C))$coefficients[2,1] # estimate initial slope
  h.est <- 1/50 # estimate handling time

  fit1 <- nls(prey.i.24.C ~ a*prey.popn.24.C/(1+a*h*prey.popn.24.C),data=subdat,start = list(a = a.est, h = h.est))
  a.fit <- summary(fit1)$parameters[1,1]
  h.fit <- summary(fit1)$parameters[2,1]

  xcoords <- seq(from = 0, to = max(subdat$prey.popn.24.C,na.rm=T),length.out = 100)
  ycoords <- a.fit*xcoords/(1+a.fit*h.fit*xcoords)
  lines(xcoords,ycoords,lwd=2,col=col2use)
}
#text(-7000,ylims[strainctr,lightctr]*.96,labels[strainctr,lightctr],pos=4,cex=1)
text(-.5,(ylims[strainctr,lightctr]-ylims.low[strainctr,lightctr])*1.12+ylims.low[strainctr,lightctr],labels[strainctr,lightctr],pos=4,cex=1.1,xpd=T)
}}
...
signif.mat <- matrix(c('a','a,b','b','a','a','a','a','a','b','a','a','a'),nrow=4,ncol=3,byrow=T)
#signif.y <- matrix(c(1.95,1.2,1.1,1.5,2.95,1.08,.55,1.03,.4,.52,.4,.32),nrow=4,ncol=3,byrow=T)
signif.y <- matrix(c(2,1.25,1.17,1.5,2.95,1.25,.57,1.03,.4,.56,.55,.43),nrow=4,ncol=3,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  boxplot(Best.a.C*10^3~EvoTemp,data=fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,],las=1,xaxt='n',ylab='',xlab='',col=EvoBgCols,border=EvoCols,ylim=c(y.lim.min[strainctr,lightctr],y.lim.max[strainctr,lightctr]))
  axis(side=1,at=c(1:3),labels=F)
  if(strainctr==2){
    if(lightctr==2){
      axis(side=1,at=c(1:3),labels=c('18°C','24°C','30°C'))
  ...
  summary.globfit <- summarySE(data = fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,], 'Best.a.C',groupvars='EvoTemp')
  points(c(1:3),summary.globfit$Best.a.C*10^3,pch=23,lwd=3,col='white')
  points(c(1:3),summary.globfit$Best.a.C*10^3,pch=23,lwd=3,col=EvoBgCols)
  points(c(1:3),summary.globfit$Best.a.C*10^3,pch=23,lwd=2,col=EvoCols)
  ...
  if(strainctr==1){
    if(lightctr==2){
      mtext(expression(paste('Clearance rate (μL · ',predator^-1,' · ',day^-1,')')),side=2,line=2.4,cex=.8,at=-0.2)
    }
  }
...
signif.mat <- matrix(c('a','a,b','b','a','b','a,b','a','a','a','a','a','a'),nrow=4,ncol=3,byrow=T)
#signif.y <- matrix(c(2.25,2.35,2.85,2.6,3.25,2.12,.04,.27,.35,.85,.342,.292),nrow=4,ncol=3,byrow=T)

#y.lim.min <- matrix(c(.8,.5,0,0),nrow=2,ncol=2,byrow=T)
#y.lim.max <- matrix(c(2.9,3.3,.36,.9),nrow=2,ncol=2,byrow=T)
y.lim.min <- matrix(c(0.04,0.04,0,0)*1000,nrow=2,ncol=2,byrow=T)
y.lim.max <- matrix(c(.22,.22,.1,.1)*1000,nrow=2,ncol=2,byrow=T)
signif.y <- matrix(c(.135,.17,.2,.13,.21,.21,.015,.03,.025,.07,.035,.03)*1000,nrow=4,ncol=3,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  if(rowcount == 3){
    boxplot(Best.h.2.C*24*1000~EvoTemp,data=fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,],las=1,xaxt='n',ylab='',xlab='',col=EvoBgCols,border=EvoCols,ylim=c(y.lim.min[strainctr,lightctr],y.lim.max[strainctr,lightctr])) #,yaxt='n'
    #axis(side = 2, at = c(0,0.1,0.2,0.3),las=1)
  } else{
  boxplot(Best.h.2.C*24*1000~EvoTemp,data=fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,],las=1,xaxt='n',ylab='',xlab='',col=EvoBgCols,border=EvoCols,ylim=c(y.lim.min[strainctr,lightctr],y.lim.max[strainctr,lightctr]))}
  axis(side=1,at=c(1:3),labels=F)
  if(strainctr==2){
    if(lightctr==2){
      axis(side=1,at=c(1:3),labels=c('18°C','24°C','30°C'))
  ...
  summary.globfit <- summarySE(data = fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,], 'Best.h.2.C',groupvars='EvoTemp')
  points(c(1:3),summary.globfit$Best.h.2.C*24*1000,pch=23,lwd=3,col='white')
  points(c(1:3),summary.globfit$Best.h.2.C*24*1000,pch=23,lwd=3,col=EvoBgCols)
  points(c(1:3),summary.globfit$Best.h.2.C*24*1000,pch=23,lwd=2,col=EvoCols)
  ...
  if(strainctr==1){
    if(lightctr==2){
      mtext(expression(paste('Handling time (predator hours · ng ',italic('Ochromonas'),' ',C^-1,')')),side=2,line=2.4,cex=.8,at=-0)
    }
  }
...
```{r}
# Significance tests: a
TukeyHSD(aov(Best.a.C~as.factor(EvoTemp),data=fits[fits$Strain==1393&fits$AssayTemp==18&fits$EvoLight==50,])) # 30 and 18 different

TukeyHSD(aov(Best.a.C~as.factor(EvoTemp),data=fits[fits$Strain==1393&fits$AssayTemp==18&fits$EvoLight==100,])) # ns

TukeyHSD(aov(Best.a.C~as.factor(EvoTemp),data=fits[fits$Strain==2951&fits$AssayTemp==18&fits$EvoLight==50,])) # all 3 different

TukeyHSD(aov(Best.a.C~as.factor(EvoTemp),data=fits[fits$Strain==2951&fits$AssayTemp==18&fits$EvoLight==100,])) # 30 and 18 different


# Significance tests: h
TukeyHSD(aov(Best.h.2.C~as.factor(EvoTemp),data=fits[fits$Strain==1393&fits$AssayTemp==18&fits$EvoLight==50,])) # 30 and 24 different

TukeyHSD(aov(Best.h.2.C~as.factor(EvoTemp),data=fits[fits$Strain==1393&fits$AssayTemp==18&fits$EvoLight==100,])) # ns

TukeyHSD(aov(Best.h.2.C~as.factor(EvoTemp),data=fits[fits$Strain==2951&fits$AssayTemp==18&fits$EvoLight==50,])) # ns

TukeyHSD(aov(Best.h.2.C~as.factor(EvoTemp),data=fits[fits$Strain==2951&fits$AssayTemp==18&fits$EvoLight==100,])) # ns

```


## Growth rates as a function of prey carbon (not prey biomass)
```{r}
# Fit linear models

fits$pred.sens.slope.C <- NaN
fits$pred.sens.slope.se.C <- NaN
fits$pred.sens.slope.tval.C <- NaN
fits$pred.sens.slope.pval.C <- NaN
fits$Glob.slope.C <- NaN
...
for(i in 1:length(expts)){
  ...
  if(fits[fits$Expt==expts[i],]$AssayTemp[1] %in% c(18,24)){
    ...
    for(j in 1:length(reps)){
      subdat2 <- subdat[subdat$Rep==reps[j],]
      # TYPE I
      lm1 <- nls(pred.sens.72~a*prey.popn.24.C,data=subdat2,start=list(a=a.est))
      fits[fits$ID == subdat2$ID[1],]$pred.sens.slope.C <- summary(lm1)$coefficients[1,1]
      fits[fits$ID == subdat2$ID[1],]$pred.sens.slope.se.C <- summary(lm1)$coefficients[1,2]
      fits[fits$ID == subdat2$ID[1],]$pred.sens.slope.tval.C <- summary(lm1)$coefficients[1,3]
      fits[fits$ID == subdat2$ID[1],]$pred.sens.slope.pval.C <- summary(lm1)$coefficients[1,4]
    
      } # End looping over reps
    
    # Global fit
    fit2 <- nls(pred.sens.72 ~ a*prey.popn.24.C,data=subdat,start = list(a = a.est))
    fits[fits$Expt == expts[i],]$Glob.slope.C <- rep(summary(fit2)$parameters[1,1],length(reps))
...
Plotting
```{r Figure2: Predator growth R2 by carbon,fig.width=5.5,fig.height=5.4}
...
plot((subdat$prey.popn.24.C),subdat$pred.sens.72,las=1,xlab='',ylab='',type='n',xlim=xlims,ylim=c(ylims.low[strainctr,lightctr],ylims[strainctr,lightctr]),xaxt='n')
axis(side=1,las=1,labels=F)
#axis(side=2,las=1,labels=F)
abline(h=0,lty=3)
...
if(lightctr==2){
  axis(side=2,las=1)
  if(strainctr==2){
    legend(2.15,0.33,c('18°C','24°C','30°C'),pch=21,col=EvoCols,pt.bg=EvoBgCols,lwd=1,bty='n',bg='white')  }
}
if(strainctr==2){
  if(lightctr==2){
    axis(side=1,las=1)
    mtext(expression(paste('Mean ',italic('Ochromonas'),' biomass (mg C · ',mL^-1,')')),side=1,line=2.4,cex=.8)
  }
}

  if(strainctr == 1){
    if(lightctr==2){
    mtext(expression(paste('Predator normalized growth rate (',d^-1,')')),side=2,line=2.6,at=-.6,cex=.8)
  }}
...
subdat <- subdat[subdat$prey.i.24>=0,]
  ...
  # RE PLOT POINTS SO THEY ARE ON TOP
  points((subdat$prey.popn.24.C),subdat$pred.sens.72,pch=pchs[as.factor(subdat$Rep)],lwd=1,col=bg2use,bg=bg2use)

# OVERALL FIT
  fit1 <- nls(pred.sens.72 ~ a*prey.popn.24.C,data=subdat,start = list(a = a.est))
  a.fit <- summary(fit1)$parameters[1,1]
  xcoords <- seq(from = 0, to = max(subdat$prey.popn.24.C,na.rm=T),length.out = 100)
  ycoords <- a.fit*xcoords
  lines(xcoords,ycoords,lwd=2,col=col2use)
}
text(-.5,(ylims[strainctr,lightctr]-ylims.low[strainctr,lightctr])*1.1+ylims.low[strainctr,lightctr],labels[strainctr,lightctr],pos=4,cex=1.1,xpd=T)
...
#box.ylims.min <- matrix(c(-3.4,-3.1,0,-.45),nrow=2,ncol=2,byrow=T)
#box.ylims.max <- matrix(c(3.25,.7,2.8,2.5),nrow=2,ncol=2,byrow=T)
box.ylims.min <- matrix(c(-.25,-.25,-.05,-.05),nrow=2,ncol=2,byrow=T)
box.ylims.max <- matrix(c(.24,.24,.28,.28),nrow=2,ncol=2,byrow=T)

# Significance letters
signif.mat <- matrix(c('a','a','a','a','a','b','a','a,b','b','a','a','a'),nrow=4,ncol=3,byrow=T)
signif.y <- matrix(c(.125,.235,.05,.05,.05,.1,.2,.18,.15,.17,.22,.27),nrow=4,ncol=3,byrow=T)
signif.t <- matrix(c('','','','*','*','','**','***','**','','**',''),nrow=4,ncol=3,byrow=T)
signif.t.y <- matrix(c(0,0,0,-.18,-.2,0,.033,.06,.02,0,-0.03,0),nrow=4,ncol=3,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  boxplot(pred.sens.slope.C~EvoTemp,data=fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,],ylim=c(box.ylims.min[strainctr,lightctr],box.ylims.max[strainctr,lightctr]),las=1,xaxt='n',yaxt='n',ylab='',xlab='',col='white',border='white');  axis(side=1,at=c(1:3),labels=F)
  abline(h=0,lty=2)
  boxplot(pred.sens.slope.C~EvoTemp,data=fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,],col=EvoBgCols,border=EvoCols,add=T,xaxt='n',yaxt='n')
  
  summary.globfit <- summarySE(data = fits[fits$Strain==StrainChoice&fits$AssayTemp==18&fits$EvoLight==EvoLightChoice,], 'Glob.slope.C',groupvars='EvoTemp')
  points(c(1:3),summary.globfit$Glob.slope.C,pch=23,lwd=3,col='white')
  points(c(1:3),summary.globfit$Glob.slope.C,pch=23,lwd=3,col=EvoBgCols)
  points(c(1:3),summary.globfit$Glob.slope.C,pch=23,lwd=2,col=EvoCols)
  ...
  if(strainctr==1){
    if(lightctr==2){
      mtext(expression(paste('Predator Growth Sensitivity (mL · mg',C^-1,' · ',d^-1,')')),side=2,line=2.8,cex=.8,at=-.35)
    }
  }
...
TukeyHSD(aov(pred.sens.slope.C ~ as.factor(EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==50,]))
summary(lm(pred.sens.slope.C ~ (EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==50,]))
t.test(fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==50 & fits$EvoTemp==18,]$pred.sens.slope.C)
t.test(fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==50 & fits$EvoTemp==24,]$pred.sens.slope.C)
t.test(fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==50 & fits$EvoTemp==30,]$pred.sens.slope.C)

TukeyHSD(aov(pred.sens.slope.C ~ as.factor(EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==100,])) # 30°C signif different from all others at p < 0.05
summary(lm(pred.sens.slope.C ~ (EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==100,])) # p = 0.02079
t.test(fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==100 & fits$EvoTemp==18,]$pred.sens.slope.C) # p = 0.04434
t.test(fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==100 & fits$EvoTemp==24,]$pred.sens.slope.C) # p = .02567
t.test(fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==100 & fits$EvoTemp==30,]$pred.sens.slope.C)

TukeyHSD(aov(pred.sens.slope.C ~ as.factor(EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==50,])) #ns
summary(lm(pred.sens.slope.C ~ (EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==50,])) #ns
t.test(fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==50 & fits$EvoTemp==18,]$pred.sens.slope.C) # p = 0.0009537 ***
t.test(fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==50 & fits$EvoTemp==24,]$pred.sens.slope.C) # p = 0.0001404 ***
t.test(fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==50 & fits$EvoTemp==30,]$pred.sens.slope.C) # p = 0.002595 **

TukeyHSD(aov(pred.sens.slope.C ~ as.factor(EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==100,])) #ns
summary(lm(pred.sens.slope.C ~ as.factor(EvoTemp) , data=fits[fits$AssayTemp==18 & fits$Strain=='1393' & fits$EvoLight==50,])) #ns
t.test(fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==100 & fits$EvoTemp==18,]$pred.sens.slope.C) # ns
t.test(fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==100 & fits$EvoTemp==24,]$pred.sens.slope.C) # p = 0.001951 **
t.test(fits[fits$AssayTemp==18 & fits$Strain=='2951' & fits$EvoLight==100 & fits$EvoTemp==30,]$pred.sens.slope.C) # ns

```


### Evolved at 24°C carbon fits
```{r}
# Significance tests: a
TukeyHSD(aov(Best.a.C~as.factor(AssayTemp),data=fits[fits$Strain==1393&fits$EvoTemp==24&fits$EvoLight==50,])) # 30 and 18 different

TukeyHSD(aov(Best.a.C~as.factor(AssayTemp),data=fits[fits$Strain==1393&fits$EvoTemp==24&fits$EvoLight==100,])) # ns

TukeyHSD(aov(Best.a.C~as.factor(AssayTemp),data=fits[fits$Strain==2951&fits$EvoTemp==24&fits$EvoLight==50,])) # all 3 different

TukeyHSD(aov(Best.a.C~as.factor(AssayTemp),data=fits[fits$Strain==2951&fits$EvoTemp==24&fits$EvoLight==100,])) # 30 and 18 different


# Significance tests: h
TukeyHSD(aov(Best.h.2.C~as.factor(AssayTemp),data=fits[fits$Strain==1393&fits$EvoTemp==24&fits$EvoLight==50 & fits$AssayTemp%in%c(18,24),])) # 30 and 24 different

TukeyHSD(aov(Best.h.2.C~as.factor(AssayTemp),data=fits[fits$Strain==1393&fits$EvoTemp==24&fits$EvoLight==100 & fits$AssayTemp%in%c(18,24),])) # ns

TukeyHSD(aov(Best.h.2.C~as.factor(AssayTemp),data=fits[fits$Strain==2951&fits$EvoTemp==24&fits$EvoLight==50 & fits$AssayTemp%in%c(18,24),])) # ns

TukeyHSD(aov(Best.h.2.C~as.factor(AssayTemp),data=fits[fits$Strain==2951&fits$EvoTemp==24&fits$EvoLight==100 & fits$AssayTemp%in%c(18,24),])) # ns
...
# Axis limits
xlims <- c(0,2.3)
ylims <- matrix(c(600,600,900,900)/1000,nrow=2,ncol=2,byrow=T)
ycoordlabel<- matrix(c(665,665,980,980)/1000,nrow=2,ncol=2,byrow=T)
...
plot((subdat$prey.popn.24.C),subdat$prey.i.24.C/1000,las=1,xlab='',ylab='',type='n',xlim=xlims,ylim=c(0,ylims[strainctr,lightctr]),xaxt='n')
axis(side=1,las=1,labels=F)
#axis(side=2,las=1,labels=F)
...
if(strainctr==2){
  if(lightctr==2){
    axis(side=1,las=1)
    mtext(expression(paste('Mean ', italic('Ochromonas'),' biomass (mg C · ',mL^-1,')')),side=1,line=2.3,cex=.8)
    legend(x='topright',inset=0,c('18°C','24°C'),pch=21,col=EvoCols,pt.bg=EvoBgCols,lwd=1,bty='n')
  }
}

  if(strainctr == 1){
    if(lightctr==2){
    mtext(expression(paste('Ingestion rate (ng ',italic('Ochromonas'),' C · ',italic('O. marina')^-1,' · ',d^-1,')')),side=2,line=2.2,at=-150/1000,cex=.8)
  }}
...
subdat <- subdat[subdat$prey.i.24.C>=0,]
  ...
  for(i in 1:length(reps)){
    ...
    if(subdat2$ID[1]%in%c('23.D','33.F')){
      #fit1 <- nls(prey.i.24.C ~ a*D1OchPop/(1+a*h*D1OchPop),data=subdat2,start = list(a = a.est, h = h.est))
    } else{ fit1 <- nls(prey.i.24.C ~ a*prey.popn.24.C/(1+a*h*prey.popn.24.C),data=subdat2,start = list(a = a.est, h = h.est))}
    ...
    if(h.fit < 0 ){ 
      s.fit <- summary(lm1)$parameters[1,1]
      xcoords <- seq(from = 0, to = max(subdat2$prey.popn.24.C,na.rm=T),length.out = 100)
      ycoords <- s.fit*xcoords/1000
      lines(xcoords,ycoords,lwd=1,col=bg2use)
    #abline(a=0,b=s.fit,col=bg2use,lwd=2)
    }else{
    xcoords <- seq(from = 0, to = max(subdat2$prey.popn.24.C,na.rm=T),length.out = 100)
    ycoords <- a.fit*xcoords/(1+a.fit*h.fit*xcoords)/1000
    lines(xcoords,ycoords,lwd=1,col=bg2use)
    }
    } else{
  ...
  # RE PLOT POINTS SO THEY ARE ON TOP
  points((subdat$prey.popn.24.C),subdat$prey.i.24.C/1000,pch=pchs[as.factor(subdat$Rep)],lwd=1,col=bg2use,bg=bg2use)

# OVERALL FIT
  a.est <- summary(lm(subdat[1:7,]$prey.i.24.C~subdat[1:7,]$prey.popn.24.C))$coefficients[2,1] # estimate initial slope
  h.est <- 1/500 # estimate handling time
  ...
  xcoords <- seq(from = 0, to = max(subdat$prey.popn.24.C,na.rm=T),length.out = 100)
  ycoords <- a.fit*xcoords/(1+a.fit*h.fit*xcoords)
  lines(xcoords,ycoords/1000,lwd=2,col=col2use)

}
#text(-7000,ylims[strainctr,lightctr]*.96,labels[strainctr,lightctr],pos=4,cex=1)
text(-0.4,ycoordlabel[strainctr,lightctr],labels[strainctr,lightctr],pos=4,cex=1.1,xpd=T)
...
ylim.up <- matrix(c(2.8,2.8,11,1),nrow=2,ncol=2,byrow=T)
ylim.down <- matrix(c(0.4,0.4,0.25,0.25),nrow=2,ncol=2,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  boxplot(Best.a.C*10^3~AssayTemp,data=fits[fits$Strain==StrainChoice&fits$EvoTemp==24&fits$EvoLight==EvoLightChoice,],las=1,xaxt='n',ylab='',xlab='',col=AssayBgCols,border=AssayCols);  axis(side=1,at=c(1:3),labels=F,ylim=c(ylim.down[strainctr,lightctr],ylim.up[strainctr,lightctr]))
  ...
  if(strainctr==1){
    if(lightctr==2){
      mtext(expression(paste('Clearance rate (μL · ',predator^-1,' · ',d^-1,')')),side=2,line=2.3,cex=.8,at=0)
    }
  }
...
ylim.up <- matrix(c(350,350,23,23),nrow=2,ncol=2,byrow=T)
ylim.down <- matrix(c(0,0,0,0),nrow=2,ncol=2,byrow=T)

signifs <- matrix(data=c('**','*','',''),nrow=2,ncol=2,byrow=T)
signifs.y <- matrix(data=c(160,210,0.9,0.85),nrow=2,ncol=2,byrow=T)
...
for(lightctr in 1:length(EvoLightChoices)){
  ...
  boxplot(Best.h.2.C*24*1000~AssayTemp,data=fits[fits$Strain==StrainChoice&fits$EvoTemp==24&fits$EvoLight==EvoLightChoice&fits$AssayTemp!=30,],las=1,xaxt='n',ylab='',xlab='',col=AssayBgCols,border=AssayCols,ylim=c(ylim.down[strainctr,lightctr],ylim.up[strainctr,lightctr]));  axis(side=1,at=c(1:3),labels=F)
  ...
  if(strainctr==1){
    if(lightctr==2){
      mtext(expression(paste('Handling time (predator hours · ng ',italic('Ochromonas'),' ',C^-1,')')),side=2,line=2.5,cex=.8,at=-1.5)
    } }
}}
